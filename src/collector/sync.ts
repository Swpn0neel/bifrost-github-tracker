...
/** Cursor keys are bare for the primary repo (as they always were) and suffixed with the name for any other. */
export function stateKey(gh: GitHubClient, base: string): string {
  return gh.fullName.toLowerCase() === env.repo.toLowerCase() ? base : `${base}:${gh.fullName}`;
}

// --- generic batched upsert -------------------------------------------------

async function upsert(table: string, columns: string[], rows: unknown[][], conflictKeys: string[]): Promise<number> {
  ...
  for (let i = 0; i < rows.length; i += CHUNK) {
    ...
    await query(
      `INSERT INTO ${table} (${columns.join(",")}) VALUES ${tuples.join(",")} ON CONFLICT (${conflictKeys.join(",")}) ${setClause}`,
      params,
    );
  }
  return rows.length;
...
const ISSUE_COLUMNS = ["repo", "number", "is_pr", "title", "author", "state", "created_at", "closed_at", "merged_at", "labels", "comments", "updated_at"];

export async function syncIssues(gh: GitHubClient, log: Log, full: boolean): Promise<number> {
  const since = full ? null : await getState(stateKey(gh, "issues_since"));
  ...
  for await (const { page, items, lastPage } of gh.pages<IssueItem>(gh.repoPath("/issues"), { params })) {
    ...
    total += await upsert("issues", ISSUE_COLUMNS, rows, ["repo", "number"]);
  ...
  if (maxUpdated) await setState(stateKey(gh, "issues_since"), maxUpdated);
  return total;
}
...
const COMMIT_COLUMNS = ["repo", "sha", "author_login", "author_name", "committed_at", "message"];

export async function syncCommits(gh: GitHubClient, log: Log, branch: string, full: boolean): Promise<number> {
  const since = full ? null : await getState(stateKey(gh, "commits_since"));
  ...
  for await (const { page, items, lastPage } of gh.pages<CommitItem>(gh.repoPath("/commits"), { params })) {
    ...
    for (const it of items) {
      ...
      if (!date) continue;
      rows.push([gh.fullName, it.sha, it.author?.login ?? null, it.commit.author?.name ?? null, date, it.commit.message.split("\n")[0].slice(0, 200)]);
      if (date > maxCommitted) maxCommitted = date;
    }
    total += await upsert("commits", COMMIT_COLUMNS, rows, ["repo", "sha"]);
    if (full || page % 10 === 0) log(`commits page ${page}/${lastPage ?? "?"} (${total} rows)`);
  }
  if (maxCommitted) await setState(stateKey(gh, "commits_since"), maxCommitted);
  return total;
}
...
const STAR_COLUMNS = ["repo", "login", "starred_at", "unstarred_at"];
...
export async function syncStargazers(gh: GitHubClient, log: Log, full: boolean): Promise<number> {
  ...
  const unavailableKey = stateKey(gh, "stargazers_unavailable");
  if ((await getState(unavailableKey)) === "true" && !full) {
  ...
  try {
    ...
    await setState(unavailableKey, "false");
  } catch (err) {
    if (err instanceof GitHubError && (err.status === 403 || err.status === 404)) {
      await setState(unavailableKey, "true");
  ...
  if (full) {
    ...
    for await (const { page, items, lastPage } of gh.pages<StarItem>(path, { accept: STAR_ACCEPT })) {
      const rows = items.filter((it) => it.user?.login).map((it) => [gh.fullName, it.user!.login, it.starred_at, null]);
      await upsert("stargazers", STAR_COLUMNS, rows, ["repo", "login"]);
      for (const r of rows) seen.push(r[1] as string);
    ...
    const gone = await query<{ login: string }>(
      `UPDATE stargazers SET unstarred_at = now()
       WHERE repo = $2 AND unstarred_at IS NULL AND NOT (login = ANY($1::text[])) RETURNING login`,
      [seen, gh.fullName],
    );
    if (gone.length) log(`stargazers: marked ${gone.length} as unstarred`);
    await setState(stateKey(gh, "stars_full_synced_at"), new Date().toISOString());
    return seen.length;
  }

  const known = await queryOne<{ n: number }>("SELECT count(*)::int AS n FROM stargazers WHERE repo = $1 AND unstarred_at IS NULL", [gh.fullName]);
  ...
  for await (const { items } of gh.pages<StarItem>(path, { accept: STAR_ACCEPT, startPage })) {
    const rows = items.filter((it) => it.user?.login).map((it) => [gh.fullName, it.user!.login, it.starred_at, null]);
    total += await upsert("stargazers", STAR_COLUMNS, rows, ["repo", "login"]);
  }
...
export async function syncForks(gh: GitHubClient, log: Log, full: boolean): Promise<number> {
  ...
  for await (const { page, items, lastPage } of gh.pages<ForkItem>(gh.repoPath("/forks"), { params: { sort: "newest" } })) {
    ...
    const known = full
      ? []
      : await query<{ fork_id: number }>("SELECT fork_id FROM forks WHERE repo = $2 AND fork_id = ANY($1::bigint[])", [ids, gh.fullName]);
    total += await upsert(
      "forks",
      ["repo", "fork_id", "owner", "created_at"],
      items.map((it) => [gh.fullName, it.id, it.owner?.login ?? null, it.created_at]),
      ["fork_id"],
    );
...
export async function syncReleases(gh: GitHubClient, log: Log, full: boolean): Promise<number> {
  ...
  for await (const { page, items, lastPage } of gh.pages<ReleaseItem>(gh.repoPath("/releases"))) {
    ...
    const known = full
      ? []
      : await query<{ release_id: number }>("SELECT release_id FROM releases WHERE repo = $2 AND release_id = ANY($1::bigint[])", [ids, gh.fullName]);
    total += await upsert(
      "releases",
      ["repo", "release_id", "tag", "name", "prerelease", "draft", "published_at"],
      items.map((it) => [gh.fullName, it.id, it.tag_name, it.name, it.prerelease, it.draft, it.published_at]),
      ["release_id"],
    );
